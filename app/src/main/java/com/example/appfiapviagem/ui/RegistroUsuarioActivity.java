                    ...
                    .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if (task.isSuccessful()) {
                                Log.d("success", "Usuario cadastrado com sucesso");
                                FirebaseUser user = mAuth.getCurrentUser();

                                Toast.makeText(getApplicationContext(), "Usuario cadastrado com sucesso", Toast.LENGTH_SHORT).show();

                            } else {
                                Log.w("error", "Problemas na autenticação do usuario", task.getException());
                                Toast.makeText(RegistroUsuarioActivity.this, "Problemas na autenticação do usuario", Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
            );
